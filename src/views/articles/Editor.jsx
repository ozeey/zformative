...
class Editor extends Component {
    ...
    componentDidMount() {
        const {match: {params: {slug}}, article} = this.props
        if(slug !== undefined ){
    ...
    saveIt = () => {
        ...
        this.stateData = { ...this.stateData, title:  this.state.title, slug: this.createSlug( this.state.title ) }
    ...
    getEditorState = (field) => {
        const {article, match: {params}} = this.props
        console.log("article",article)
        if(article == null || article === undefined || params === undefined || params === null) return null
        const {slug} = params
    ...
    render() {
        ...
        return (
            <div className="add-editor-wrapper">
                <EditableH1 value={this.state.title} onSave={(val)=>this.onChangeTitle(val)} />
                <br />
                <small>{this.state.slug}</small>
                <Editor onChange={this.onChangeExcerpt} {...this.props} editorState={editorStateExcerpt}/>
                <Editor onChange={this.onChangeBody} {...this.props} editorState={editorStateBody}/>
...
const mapStateToProps = ({root: {articles: {article, error, isLoading} = []} = []}) => {
    return {
